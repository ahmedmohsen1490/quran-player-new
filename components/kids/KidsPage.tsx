...
import { KidsProgress, KidsStory, KidsGame, KidsDhikr, FlashcardSet, KidsQuizQuestion, KidsMatchingPair, KidProfile, AgeGroup } from '../../types';
...
const HeroesIcon = () => <span className="text-4xl">🦸‍♂️</span>;
const FiqhYouthIcon = () => <span className="text-4xl">📜</span>;

const SoundIcon = () => (
    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" /></svg>
);

type View = 'menu' | 'stories' | 'story_detail' | 'games' | 'game_detail' | 'dhikr' | 'flashcards' | 'flashcard_set' | 'heroes';


// --- THEMES ---
const themes = {
  'boy-young': { bg: 'bg-sky-100', primary: 'text-sky-600', accent: 'bg-sky-200' },
  'girl-young': { bg: 'bg-pink-100', primary: 'text-pink-600', accent: 'bg-pink-200' },
  'boy-explorer': { bg: 'bg-teal-100', primary: 'text-teal-700', accent: 'bg-teal-200' },
  'girl-explorer': { bg: 'bg-purple-100', primary: 'text-purple-700', accent: 'bg-purple-200' },
  'boy-gem': { bg: 'bg-blue-100', primary: 'text-blue-800', accent: 'bg-blue-200' },
  'girl-gem': { bg: 'bg-fuchsia-100', primary: 'text-fuchsia-800', accent: 'bg-fuchsia-200' },
};

const getThemeKey = (profile: KidProfile): keyof typeof themes => {
    if (profile.age <= 5) return profile.gender === 'male' ? 'boy-young' : 'girl-young';
    if (profile.age <= 12) return profile.gender === 'male' ? 'boy-explorer' : 'girl-explorer';
    return profile.gender === 'male' ? 'boy-gem' : 'girl-gem';
};
...
const getVisibleContent = <T extends { ageGroup: AgeGroup }>(allItems: T[], age: number): T[] => {
    const visibleAgeGroups: AgeGroup[] = [];
    if (age <= 5) visibleAgeGroups.push('3-5');
    if (age >= 6 && age <= 8) visibleAgeGroups.push('3-5', '6-8');
    if (age >= 9 && age <= 12) visibleAgeGroups.push('6-8', '9-12');
    if (age >= 13) visibleAgeGroups.push('9-12', '13-15');
...
const StoriesView: React.FC<{ stories: KidsStory[]; progress: KidsProgress; onComplete: (storyId: number) => void; onBack: () => void; theme: typeof themes[keyof typeof themes] }> = ({ stories, progress, onComplete, onBack, theme }) => {
            ...
            <div>
                ...
                <div className="bg-card p-6 rounded-lg shadow-lg">
                    ...
                    <div className="flex justify-between items-center mb-4">
                        <h3 className={`text-2xl font-bold ${theme.primary} font-tajawal`}>{selectedStory.title}</h3>
                        <button 
                            onClick={() => speak(selectedStory.title + ". " + selectedStory.content)} 
                            className={`p-2 ${theme.primary} hover:${theme.accent} rounded-full flex items-center gap-2`}
    ...
    return (
        <div>
            ...
            <h2 className={`text-2xl font-bold ${theme.primary} mb-4 font-tajawal`}>قصص القرآن المبسطة</h2>
            {stories.length > 0 ? (
...
const QuizGame: React.FC<{ game: KidsGame; progress: number[]; onComplete: (questionId: number) => void; theme: typeof themes[keyof typeof themes] }> = ({ game, progress, onComplete, theme }) => {
    ...
    if (!currentQuestion) {
        return <div className="bg-card p-6 rounded-lg text-center font-tajawal"><h3 className={`text-xl font-bold ${theme.primary}`}>رائع! لقد أكملت كل الأسئلة في هذه اللعبة.</h3></div>;
    }
    ...
    return (
        <div className="bg-card p-6 rounded-lg shadow-lg font-tajawal">
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => speak(currentQuestion.question)} className={`p-2 ${theme.primary} hover:${theme.accent} rounded-full flex-shrink-0`}><SoundIcon /></button>
            </div>
            <div className="space-y-3">
            ...
            {isAnswered && (
                <div className="text-center mt-4 animate-fade-in">
                    ...
                    <button onClick={handleNext} className={`mt-2 ${theme.accent} ${theme.primary} font-bold py-2 px-6 rounded-md`}>
...
const MatchingGame: React.FC<{ game: KidsGame; progress: number[]; onComplete: (pairId: number) => void; theme: typeof themes[keyof typeof themes] }> = ({ game, progress, onComplete, theme }) => {
    ...
    if(matchedIds.length === pairs.length){
         return <div className="bg-card p-6 rounded-lg text-center font-tajawal"><h3 className={`text-xl font-bold ${theme.primary}`}>أحسنت! لقد طابقت كل البطاقات بنجاح.</h3></div>;
    }
    ...
    const getButtonClass = (item: KidsMatchingPair, isCol1: boolean) => {
        ...
        if (isSelected) return `ring-2 ring-primary ${incorrect ? 'bg-red-200' : theme.accent}`;
        return 'bg-card hover:bg-border-color';
    };

    return (
        <div className="bg-card p-4 rounded-lg shadow-lg font-tajawal">
             <h3 className={`text-xl font-bold text-center ${theme.primary} mb-4`}>طابق بين العمودين</h3>
...
const GamesView: React.FC<{ games: KidsGame[]; progress: Record<string, number[]>; onComplete: (gameId: string, questionId: number) => void; onBack: () => void; theme: typeof themes[keyof typeof themes] }> = ({ games, progress, onComplete, onBack, theme }) => {
    ...
    if (selectedGame) {
        return (
            <div>
                 ...
                 {selectedGame.type === 'quiz' && <QuizGame game={selectedGame} progress={progress[selectedGame.id] || []} onComplete={(qId) => onComplete(selectedGame.id, qId)} theme={theme} />}
                 {selectedGame.type === 'matching' && <MatchingGame game={selectedGame} progress={progress[selectedGame.id] || []} onComplete={(pId) => onComplete(selectedGame.id, pId)} theme={theme} />}
            </div>
        )
    }
    ...
    return (
        <div>
            <button onClick={onBack} className="flex items-center gap-2 text-text-secondary hover:text-text-primary mb-4 font-tajawal"><BackIcon className="w-5 h-5" /><span>العودة للقائمة</span></button>
            <h2 className={`text-2xl font-bold ${theme.primary} mb-4 font-tajawal`}>الألعاب التعليمية</h2>
            {games.length > 0 ? (
                <div className="space-y-3">
                    {games.map(game => (
...
const DhikrView: React.FC<{ dhikrs: KidsDhikr[]; progress: string[]; onComplete: (dhikrId: string) => void; onBack: () => void; theme: typeof themes[keyof typeof themes] }> = ({ dhikrs, progress, onComplete, onBack, theme }) => {
    return (
         <div>
            <button onClick={onBack} className="flex items-center gap-2 text-text-secondary hover:text-text-primary mb-4 font-tajawal"><BackIcon className="w-5 h-5" /><span>العودة للقائمة</span></button>
            <h2 className={`text-2xl font-bold ${theme.primary} mb-4 font-tajawal`}>أذكار الطفل المسلم</h2>
            {dhikrs.length > 0 ? (
              <div className="space-y-3">
                  {dhikrs.map(dhikr => {
                          ...
                          <div key={dhikr.id} className="bg-card p-4 rounded-lg shadow-md">
                              ...
                              <p className="text-sm text-green-700 bg-green-100 p-2 rounded-md font-tajawal">💡 {dhikr.benefit}</p>
                               <div className="flex justify-end items-center gap-2 mt-3">
                                  <button onClick={() => speak(dhikr.dhikr)} className={`p-2 ${theme.primary} hover:${theme.accent} rounded-full`} aria-label="استمع للذكر"><SoundIcon /></button>
                                  <button onClick={() => onComplete(dhikr.id)} disabled={isCompleted} className={`px-4 py-2 text-sm font-bold rounded-md transition-colors ${isCompleted ? 'bg-green-200 text-green-800 cursor-not-allowed' : `${theme.accent} ${theme.primary} hover:bg-opacity-80`}`}>
...
const FlashcardsView: React.FC<{ setId: string; onBack: () => void; progress: boolean; onComplete: () => void; theme: typeof themes[keyof typeof themes] }> = ({ setId, onBack, progress, onComplete, theme }) => {
    ...
    return (
        <div className="font-tajawal">
            <button onClick={onBack} className="flex items-center gap-2 text-text-secondary hover:text-text-primary mb-4"><BackIcon className="w-5 h-5" /><span>العودة للبطاقات</span></button>
             <h2 className={`text-2xl font-bold ${theme.primary} mb-4 text-center`}>{set.title}</h2>
            ...
            <div className="perspective-1000">
                <div onClick={() => setIsFlipped(!isFlipped)} className={`relative w-full h-48 sm:h-64 transition-transform duration-700 transform-style-3d cursor-pointer ${isFlipped ? 'rotate-y-180' : ''}`}>
                    <div className="absolute w-full h-full backface-hidden bg-card rounded-xl shadow-lg flex items-center justify-center">
                     ...
                     <div className={`absolute w-full h-full backface-hidden text-white rounded-xl shadow-lg flex items-center justify-center rotate-y-180 ${theme.accent.replace('bg-','bg-').replace('-200', '-500')}`}>
                        <p className="text-4xl font-tajawal p-4 text-center">{card.back}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-between items-center mt-6">
             ...
             <div className="mt-6 text-center">
                <button onClick={onComplete} disabled={progress} className={`px-6 py-3 text-lg font-bold rounded-md transition-colors ${progress ? 'bg-green-200 text-green-800 cursor-not-allowed' : `${theme.accent} ${theme.primary} hover:bg-opacity-80`}`}>
...
const ProfileManager: React.FC<{
  ...
  onAddProfile: (name: string, age: number, gender: 'male' | 'female') => void;
  onDeleteProfile: (profileId: number) => void;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (name.trim() && numAge >= 3 && numAge <= 15) {
            onAddProfile(name.trim(), numAge, gender);
            setName('');
            setAge(5);
            setError('');
        } else {
            setError('الرجاء إدخال اسم صحيح وعمر بين 3 و 15 سنة.');
        }
    };
                    ...
                    {profiles.length > 0 ? (
                        profiles.map(p => (
                            <div key={p.id} className="flex items-center justify-between bg-background p-3 rounded-md">
                                <button onClick={() => onSelectProfile(p)} className="flex-grow text-right hover:text-primary flex items-center gap-3">
                                    <span className={`text-2xl`}>{p.gender === 'male' ? '👦' : '👧'}</span>
                                    <div>
                ...
                <form onSubmit={handleSubmit} className="border-t border-border-color pt-4">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-4">
                             <label className="font-semibold">العمر:</label>
                             <input
                                type="number"
                                value={age}
                        ...
                        <p className="text-xs text-text-secondary -mt-2">العمر المسموح به من 3 إلى 15 سنة.</p>

                        <div className="flex items-center gap-4">
                           <label className="font-semibold">النوع:</label>
                           <div className="flex gap-4">
                               <label className="flex items-center gap-2 cursor-pointer">
                                   <input type="radio" name="gender" value="male" checked={gender === 'male'} onChange={() => setGender('male')} className="form-radio text-primary focus:ring-primary"/>
                                   <span>ذكر 👦</span>
                               </label>
                               <label className="flex items-center gap-2 cursor-pointer">
                                   <input type="radio" name="gender" value="female" checked={gender === 'female'} onChange={() => setGender('female')} className="form-radio text-pink-500 focus:ring-pink-500"/>
                                   <span>أنثى 👧</span>
                               </label>
                           </div>
                        </div>
...
const KidsPage: React.FC = () => {
  ...
  const themeKey = useMemo(() => activeProfile ? getThemeKey(activeProfile) : 'boy-young', [activeProfile]);
  const theme = themes[themeKey];

  const handleAddProfile = (name: string, age: number, gender: 'male' | 'female') => {
    const newProfile: KidProfile = { id: Date.now(), name, age, gender };
  ...
  const visibleStories = useMemo(() => activeProfile ? getVisibleContent(kidsStories.filter(s => s.ageGroup !== '13-15'), activeProfile.age) : [], [activeProfile]);
  const visibleHeroes = useMemo(() => activeProfile ? getVisibleContent(kidsStories.filter(s => s.ageGroup === '13-15' || s.ageGroup === '9-12'), activeProfile.age) : [], [activeProfile]);
  ...
  const totalActivities = visibleStories.length + visibleHeroes.length + visibleGames.reduce((acc, game) => acc + (game.questions?.length || game.matchingPairs?.length || 0), 0) + visibleDhikrs.length + visibleFlashcardSets.length;
  ...
  const menuItems = useMemo(() => {
    ...
    let items = [
        { id: 'stories', icon: <StoryIcon />, label: 'قصص القرآن', view: 'stories', condition: age <= 12 },
        { id: 'heroes', icon: <HeroesIcon />, label: 'أبطال الإسلام', view: 'heroes', condition: age >= 9 },
        { id: 'games', icon: <GameIcon />, label: 'ألعاب تعليمية', view: 'games', condition: true },
        { id: 'dhikr', icon: <DhikrIcon />, label: 'أذكار الطفل', view: 'dhikr', condition: true },
        { id: 'flashcards', icon: <FlashcardIcon />, label: 'بطاقات تعليمية', view: 'flashcards', condition: true },
    ];
  ...
  if (!activeProfile) {
      return (
          <div className="transition-colors duration-300 bg-sky-100">
              <ProfileManager 
                  profiles={profiles}
    ...
    <>
        <div className="bg-card p-4 rounded-xl shadow-lg mb-6">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className={`text-2xl font-bold font-tajawal ${theme.primary}`}>أهلاً بك, {activeProfile.name}!</h2>
            ...
            <div className="mt-4">
                ...
                <div className="w-full bg-border-color rounded-full h-4">
                    <div className={`${theme.accent.replace('-200', '-500')} h-4 rounded-full text-white text-xs flex items-center justify-center transition-all duration-500`} style={{ width: `${overallProgress}%` }}>
        ...
        <div className="grid grid-cols-2 gap-4">
            {menuItems.map(item => (
                 <button key={item.id} onClick={() => setView(item.view as View)} className="p-4 bg-card rounded-lg shadow-md flex flex-col items-center justify-center aspect-square hover:shadow-lg transition-shadow">
                    {item.icon}
                    <span className="font-bold mt-2 font-tajawal text-center">{item.label}</span>
                </button>
            ))}
        </div>
    </>
  ...
  const renderContent = () => {
    switch (view) {
        case 'stories': return <StoriesView stories={visibleStories} progress={progress} onComplete={handleCompleteStory} onBack={() => setView('menu')} theme={theme} />;
        case 'heroes': return <StoriesView stories={visibleHeroes} progress={progress} onComplete={handleCompleteStory} onBack={() => setView('menu')} theme={theme} />;
        case 'games': return <GamesView games={visibleGames} progress={progress.gamesCorrectAnswers} onComplete={handleCompleteGameQuestion} onBack={() => setView('menu')} theme={theme} />;
        case 'dhikr': return <DhikrView dhikrs={visibleDhikrs} progress={progress.dhikrCompleted} onComplete={handleCompleteDhikr} onBack={() => setView('menu')} theme={theme} />;
        case 'flashcards': return (
            <div>
                 ...
                 <h2 className={`text-2xl font-bold ${theme.primary} mb-4 font-tajawal`}>البطاقات التعليمية</h2>
        ...
        case 'flashcard_set': return selectedSetId && <FlashcardsView setId={selectedSetId} onBack={() => setView('flashcards')} progress={progress.flashcardsCompleted.includes(selectedSetId)} onComplete={() => handleCompleteFlashcardSet(selectedSetId)} theme={theme} />;
        default: return renderMenu();
    }
  };

  return (
    <div className={`transition-colors duration-500 ${theme.bg}`}>
        <style>{`
